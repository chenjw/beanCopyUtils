...
/**
 * Javassist实现的list->pojo的copier
 * 
 * @author chenjw 2012-6-13 下午5:00:25
 */
public class JavassistListToPojoCopierFactory implements CopierFactory {
	...
	private static class MapToPojoCopierBuilder {
		...
		private void addCopyCode(ClassGenerator clazzGenerator,
				...
				Field destField, Expression converter, int getterIndex,
			...
			Expression getterInvoke = new InvokeExpression(
					CopierConstants.LIST_GET_METHOD, originField,
					new Expression[] {new Expression(String.valueOf(getterIndex),int.class)});
		...
		private void initCopyMethodBody(MethodGenerator methodGenerator) {
			...
			for (int i=0;i<setters.length;i++) {
				...
				if (setterNameToGetterNameMap != null) {
				    String getterIndexStr=setterNameToGetterNameMap
                    .get(setter.getName());
				    if(getterIndexStr!=null){
				        getterIndex = Integer.parseInt(getterIndexStr);
				    }
