...
public class Expression {
	...
	public Expression box(Class<?> destClazz) {
		...
		if (originClazz == boolean.class) {
			return new Expression("new Boolean(" + getCode() + ")",
					Boolean.class);
		} else if (originClazz == byte.class) {
			return new Expression("new Byte(" + getCode() + ")", Byte.class);
		} else if (originClazz == char.class) {
			return new Expression("new Character(" + getCode() + ")",
					Character.class);
		} else if (originClazz == double.class) {
			return new Expression("new Double(" + getCode() + ")", Double.class);
		} else if (originClazz == float.class) {
			return new Expression("new Float(" + getCode() + ")", Float.class);
		} else if (originClazz == int.class) {
			return new Expression("new Integer(" + getCode() + ")",
					Integer.class);
		} else if (originClazz == long.class) {
			return new Expression("new Long(" + getCode() + ")", Long.class);
		} else if (originClazz == short.class) {
			return new Expression("new Short(" + getCode() + ")", Short.class);
		}
		return this;
